...
export const FileTree: React.FC<FileTreeProps> = ({
    ...
    const renderNode = (node: TreeNode, depth: number = 0): React.ReactNode => {
        ...
        return (
            <div key={node.path}>
                <div
                    className={`group flex items-center gap-1 py-1 px-2 cursor-pointer transition-colors hover:bg-white/5 ${isSelected ? 'bg-blue-600/20 text-blue-400' : 'text-zinc-400 hover:text-zinc-200'}`}
                ...
                >
                    <div className="flex items-center gap-1 flex-1 min-w-0">
                        {isFolder ? (
                            <div className="shrink-0 text-zinc-600 group-hover:text-zinc-400">
                        ...
                        {!isFolder && (
                            <button
                                ...
                                className={`shrink-0 transition-colors mr-1 ${isReviewed ? 'text-green-500' : 'text-zinc-600 group-hover:text-zinc-400'}`}
                            >
...
function ChangeIcon({ type }: { type: string }) {
    switch (type) {
        ...
        case "edit": return <FileEdit className="h-4 w-4 text-blue-400" />;
